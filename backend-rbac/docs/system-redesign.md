...
HTTP Request
  ...
  → Repository（显式 db.Scopes(TenantScope(tenantID)) → GORM 查询）
...
// 使用 GORM 原生 Scopes 模式，在 Repository 中显式调用
...
**注意**：推荐在 Repository 的 Create 方法中直接赋值 `user.TenantID = xcontext.GetTenantID(ctx)`，比调用 SetTenantIDFromCtx 更简单直观。TenantScoped 接口作为可选的安全网。

**使用方式**：

```go
// 查询：使用 GORM 原生 Scopes 添加租户过滤
tenantID := xcontext.GetTenantID(ctx)
r.db.WithContext(ctx).Scopes(database.TenantScope(tenantID)).
    Where("user_id = ?", userID).First(&user)

// 创建：直接赋值 tenant_id（推荐方式，类型安全、零反射开销）
...
> **注意**：以下 DDL 中，`user_roles`、`role_permissions` 和 `roles.parent_role_id` 为本次新增/变更。其余已有表的 DDL 为参考概要，完整准确的 DDL 以 `migrations/` 目录中的迁移文件为准。
...
-- =====================================================
-- 用户岗位关联表（全局表，无 tenant_id）
-- =====================================================
CREATE TABLE user_positions (
    id          BIGSERIAL    PRIMARY KEY,
    user_id     VARCHAR(20)  NOT NULL REFERENCES users(user_id),
    position_id VARCHAR(20)  NOT NULL REFERENCES positions(position_id),
    created_at  BIGINT       NOT NULL DEFAULT 0,
...
CREATE INDEX idx_device_playlists_tenant ON device_playlists(tenant_id);

-- =====================================================
-- 设备播放列表项表（全局表，通过 playlist_id 间接关联租户）
-- =====================================================
CREATE TABLE device_playlist_items (
    id          BIGSERIAL    PRIMARY KEY,
    playlist_id VARCHAR(20)  NOT NULL REFERENCES device_playlists(playlist_id),
    video_id    VARCHAR(20)  NOT NULL DEFAULT '',
    sort        INT          NOT NULL DEFAULT 0,
    created_at  BIGINT       NOT NULL DEFAULT 0
);
CREATE INDEX idx_device_playlist_items_playlist ON device_playlist_items(playlist_id);
...
CREATE TABLE device_approvals (
    ...
    applicant_id  VARCHAR(20)   NOT NULL DEFAULT '',
    approver_id   VARCHAR(20)   NOT NULL DEFAULT '',
    status        SMALLINT      NOT NULL DEFAULT 1,
    remark        TEXT          NOT NULL DEFAULT '',
    created_at    BIGINT        NOT NULL DEFAULT 0,
    updated_at    BIGINT        NOT NULL DEFAULT 0,
...
CREATE INDEX idx_device_approvals_tenant ON device_approvals(tenant_id);
...
CREATE INDEX idx_video_files_tenant ON video_files(tenant_id);

-- =====================================================
-- 视频风险表（租户隔离）
-- =====================================================
CREATE TABLE video_risks (
...
CREATE INDEX idx_video_risks_tenant ON video_risks(tenant_id);
...
CREATE INDEX idx_video_signatures_tenant ON video_signatures(tenant_id);

-- =====================================================
-- 视频风险操作表（租户隔离，无软删除）
-- =====================================================
CREATE TABLE video_risk_operations (
    operation_id  VARCHAR(20)   PRIMARY KEY,
    tenant_id     VARCHAR(20)   NOT NULL,
    risk_id       VARCHAR(20)   NOT NULL DEFAULT '',
    operator_id   VARCHAR(20)   NOT NULL DEFAULT '',
    operation_type VARCHAR(50)  NOT NULL DEFAULT '',
    remark        TEXT          NOT NULL DEFAULT '',
    created_at    BIGINT        NOT NULL DEFAULT 0
);
CREATE INDEX idx_video_risk_operations_tenant ON video_risk_operations(tenant_id);
```
...
// Create 创建用户 — 直接赋值 tenant_id（类型安全，零反射开销）
...
func (r *UserRepo) GetByID(ctx context.Context, userID string) (*model.User, error) {
    ...
    err := r.db.WithContext(ctx).
        Scopes(database.TenantScope(xcontext.GetTenantID(ctx))).
...
| 操作 | 方式 | 说明 |
|------|------|------|
| 租户内查询 | `db.Scopes(database.TenantScope(tenantID))` | GORM 原生 Scopes，显式添加 `WHERE tenant_id = ?` |
| 跨租户查询 | 直接 `r.db.WithContext(ctx)` | 不加 TenantScope |
| 创建记录 | `model.TenantID = xcontext.GetTenantID(ctx)` | 直接赋值，类型安全，零反射开销 |
| 更新/删除 | `db.Scopes(database.TenantScope(tenantID))` + Where | 确保只操作本租户数据 |
...
type PermissionCache struct {
    ...
    ttl         time.Duration
    lastLoad    time.Time
    refreshChan chan struct{}               // 即时失效通知
}
...
func (c *PermissionCache) Refresh(ctx context.Context) error {
    ...
    // 使用递归 CTE 构建 descendant→ancestor 映射，使子角色继承所有祖先角色的权限
    ...
    err := c.db.WithContext(ctx).Raw(`
        WITH RECURSIVE role_ancestors AS (
            -- 基础：每个角色是自身的祖先（直接权限）
            SELECT role_id AS descendant, role_id AS ancestor
            FROM roles WHERE deleted_at = 0
            UNION ALL
            -- 递归：沿 parent_role_id 向上追溯所有祖先
            SELECT ra.descendant, r.parent_role_id AS ancestor
            FROM role_ancestors ra
            JOIN roles r ON r.role_id = ra.ancestor
            WHERE r.parent_role_id != '' AND r.deleted_at = 0
        )
        SELECT DISTINCT ra.descendant AS role_id, p.resource, p.action
        FROM role_ancestors ra
        JOIN role_permissions rp ON rp.role_id = ra.ancestor
    ...
    err = c.db.WithContext(ctx).Raw(`
        ...
        SELECT DISTINCT ra.descendant AS role_id, p.resource
        FROM role_ancestors ra
        JOIN role_permissions rp ON rp.role_id = ra.ancestor
        JOIN permissions p ON p.permission_id = rp.permission_id AND p.deleted_at = 0
        WHERE p.type = 'MENU'
...
// matchPath 路径匹配（兼容 Casbin keyMatch2 能力）
// 支持：精确匹配、:param 单段匹配、* 单段通配、** 多段通配
...
保留 `pkg/audit/`，写入时改用显式 `model.TenantID = xcontext.GetTenantID(ctx)`。

**重要**：TenantScope 只适用于带 `tenant_id` 列的表（用户、角色、部门、岗位、日志等）。全局表（menus、permissions、tenants、user_positions）不需要也不应使用 TenantScope。Repository 方法命名约定：
- 需要 TenantScope 的方法：`GetByID`, `Update`, `Delete`, `List`, `ListWithFilters`
- 不需要 TenantScope 的方法（跨租户/全局表）：`GetByEmail`, `GetByCode`, `GetAll`
...
1. 创建 `pkg/database/tenant.go`（TenantScope GORM 原生 Scopes + TenantScoped 接口）
2. 移除 `pkg/database/scopes.go`（GORM Callback）
3. 更新 `pkg/database/postgres.go`（移除 RegisterCallbacks 调用）
4. 重写所有 Repository（显式 Scopes 模式，Create 中直接赋值 tenant_id）
...
3. 创建 `internal/rbac/cache.go`（PermissionCache，含正确的 descendant→ancestor CTE + 即时失效）
4. 创建 `internal/middleware/rbac.go`（RBAC 中间件）
5. 重写 `role_service.go`（权限分配改为操作 role_permissions 表，变更后调用 NotifyRefresh）
...
2. 删除废弃包（casbin、rsapwd、cache）
3. 修复 CORS 配置（从配置文件读取允许域名列表）
...
// PermissionCache 在 role_service 中使用示例：
// func (s *RoleService) AssignPermissions(...) error {
//     // ... 分配权限到 role_permissions 表 ...
//     s.cache.NotifyRefresh()  // 立即触发缓存刷新
// }
...
> **总结**：新系统通过四个核心改进解决当前架构问题：
> 1. **GORM 原生 Scopes TenantScope** 替代隐式 Callback — AI 可见、无冲突、idiomatic GORM
> 2. **纯数据库 RBAC** 替代 Casbin — 减少 40% 代码、正确的 CTE 递归继承、支持 SQL JOIN
> 3. **保留 GORM + Gen** — 动态 WHERE、自动时间戳、自动软删除的优势不变
> 4. **即时缓存失效** — channel 通知 + TTL 兜底，权限变更实时生效
