...
import (
	"admin/internal/dto"
	operationlogsvc "admin/internal/service/operationlog"
	"admin/pkg/response"
...
func NewHandler(db *gorm.DB) *Handler {
	return &Handler{
		svc: operationlogsvc.NewService(db),
	}
}
...
func (h *Handler) GetOperationLog(c *gin.Context) {
	...
	resp, err := h.svc.GetOperationLogByID(c.Request.Context(), req.LogID)
	if err != nil {
		response.Error(c, err)
		return
	}
...
func (h *Handler) ListOperationLogs(c *gin.Context) {
	...
	resp, err := h.svc.ListOperationLogs(c.Request.Context(), &req)
	if err != nil {
		response.Error(c, err)
		return
	}
