...
func (r *DictItemRepo) Create(ctx context.Context, dictItem *model.DictItem) error {
	dictItem.TenantID = xcontext.GetTenantID(ctx)
...
func (r *DictItemRepo) GetByID(ctx context.Context, itemID string) (*model.DictItem, error) {
	...
	return r.q.DictItem.WithContext(ctx).
		...
		Where(r.q.DictItem.ItemID.Eq(itemID)).
		First()
}

// GetByTypeAndTenant 获取指定类型和租户的字典项（跨租户查询）
...
// GetByTypeAndValue 获取指定类型、租户、值的字典项（跨租户查询）
...
// DeleteByTypeAndValue 删除指定类型、租户、值的字典项（跨租户查询）
