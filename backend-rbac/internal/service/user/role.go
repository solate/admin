...
// NewRoleService 创建用户角色服务
func NewRoleService(db *gorm.DB, recorder *audit.Recorder) *RoleService {
	return &RoleService{
...
func (s *RoleService) AssignRoles(ctx context.Context, userID string, req *dto.AssignRolesRequest) (err error) {
	var user *model.User
...
func (s *RoleService) getUserRoles(ctx context.Context, userID, tenantID string) ([]*model.Role, error) {
...
func (s *RoleService) ValidateAndAssignRoles(ctx context.Context, userID string, roleCodes []string, tenantID string) ([]*model.Role, error) {
	if len(roleCodes) == 0 {
		return []*model.Role{}, nil
	}
...
func (s *RoleService) GetUserRoleDetails(ctx context.Context, userID, tenantID string) ([]*model.Role, error) {
	if tenantID == "" {
		log.Error().Msg("tenantID 不能为空")
		return nil, xerr.Wrap(xerr.ErrInvalidParams.Code, "tenantID 不能为空", nil)
	}
