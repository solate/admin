...
// CreateTenant 创建租户（返回创建的租户模型用于审计日志）
func (s *TenantService) CreateTenant(ctx context.Context, req *dto.TenantCreateRequest) (*model.Tenant, error) {
	// 检查租户编码是否已存在
...
func (s *TenantService) GetTenantByID(ctx context.Context, tenantID string) (*model.Tenant, error) {
...
// UpdateTenant 更新租户（返回更新前后的租户模型用于审计日志）
func (s *TenantService) UpdateTenant(ctx context.Context, tenantID string, req *dto.TenantUpdateRequest) (*model.Tenant, *model.Tenant, error) {
	// 获取旧租户信息（用于审计日志）
	oldTenant, err := s.tenantRepo.GetByID(ctx, tenantID)
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn().Str("tenant_id", tenantID).Msg("租户不存在")
			return nil, nil, xerr.ErrNotFound
		}
		log.Error().Err(err).Str("tenant_id", tenantID).Msg("查询租户失败")
		return nil, nil, xerr.Wrap(xerr.ErrInternal.Code, "查询租户失败", err)
	}
	...
	if err := s.tenantRepo.Update(ctx, tenantID, updates); err != nil {
		...
		return nil, nil, xerr.Wrap(xerr.ErrInternal.Code, "更新租户失败", err)
	}
	...
	if err != nil {
		...
		return nil, nil, xerr.Wrap(xerr.ErrInternal.Code, "获取更新后租户信息失败", err)
	}

	return oldTenant, newTenant, nil
}

// DeleteTenant 删除租户（返回删除的租户模型用于审计日志）
func (s *TenantService) DeleteTenant(ctx context.Context, tenantID string) (*model.Tenant, error) {
	// 获取租户信息（用于审计日志）
	tenant, err := s.tenantRepo.GetByID(ctx, tenantID)
	if err != nil {
		if err == gorm.ErrRecordNotFound {
	...
	if err := s.tenantRepo.Delete(ctx, tenantID); err != nil {
		...
		return nil, xerr.Wrap(xerr.ErrInternal.Code, "删除租户失败", err)
	}
...
// ListTenants 获取租户列表（返回模型列表和总数）
func (s *TenantService) ListTenants(ctx context.Context, req *dto.TenantListRequest) ([]*model.Tenant, int64, error) {
	// 获取租户列表和总数，支持筛选条件
	...
	if err != nil {
		log.Error().Err(err).
			...
			Msg("查询租户列表失败")
		return nil, 0, xerr.Wrap(xerr.ErrInternal.Code, "查询租户列表失败", err)
	}
