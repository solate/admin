...
import (
	"admin/internal/dto"
	loginlogsvc "admin/internal/service/loginlog"
	"admin/pkg/response"
...
// Handler 登录日志处理器
type Handler struct {
	svc *loginlogsvc.Service
}

// NewHandler 创建登录日志处理器
func NewHandler(db *gorm.DB) *Handler {
	return &Handler{
		svc: loginlogsvc.NewService(db),
	}
}
...
func (h *Handler) GetLoginLog(c *gin.Context) {
	...
	resp, err := h.svc.GetLoginLogByID(c.Request.Context(), req.LogID)
	if err != nil {
		response.Error(c, err)
		return
	}
...
func (h *Handler) ListLoginLogs(c *gin.Context) {
	...
	resp, err := h.svc.ListLoginLogs(c.Request.Context(), &req)
	if err != nil {
		response.Error(c, err)
		return
	}
