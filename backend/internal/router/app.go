...
func (s *App) initHandlers() error {
	...
	// 初始化服务层
	authService := service.NewAuthService(userRepo, userRoleRepo, s.JWT, s.Redis, s.Config) // 初始化认证服务
	...
	roleService := service.NewRoleService(roleRepo, userRoleRepo)                                                 // 初始化角色服务
