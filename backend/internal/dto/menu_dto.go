...
type CreateMenuRequest struct {
	...
	Type      string `json:"type" binding:"required,oneof=MENU BUTTON"` // 类型: MENU或BUTTON
	...
	Action    string `json:"action" binding:"omitempty"`          // 操作类型(仅BUTTON有效)
	Resource  string `json:"resource" binding:"omitempty"`        // 资源路径
}
...
type UpdateMenuRequest struct {
	...
	Sort      *int16 `json:"sort" binding:"omitempty"`             // 排序
	Status    int    `json:"status" binding:"omitempty,oneof=1 2"` // 状态 1:显示 2:隐藏
	Action    string `json:"action" binding:"omitempty"`           // 操作类型
	Resource  string `json:"resource" binding:"omitempty"`         // 资源路径
}

// MenuInfo 菜单信息
type MenuInfo struct {
	PermissionID string  `json:"permission_id"` // 菜单ID
