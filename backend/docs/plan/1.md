...
**说明**：
- 表名改为 `tenant_menus`（复数形式，与 SQL 一致）
- 字段 `template_id` 改为 `menu_id`（与 SQL 一致）
- 移除 `is_enabled`、`assigned_at`、`assigned_by` 字段（简化设计）
...
- [ ] 新建 `tenant_menus` 表（租户菜单边界，与 SQL 一致）
