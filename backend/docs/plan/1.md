# 多租户SaaS后端管理系统设计

## 核心设计原则

**分配机制**：超管创建系统模板 → 勾选分配给租户 → 租户看到被分配的数据

**角色层级**：
- 超级管理员 (SuperAdmin)：管理平台模板，分配给租户
- 租户管理员 (TenantAdmin)：继承系统模板，添加自定义数据
- 普通用户 (User)：使用被授权的功能

---

## 1. 数据库设计

### 1.1 用户角色系统

> 详细设计见 [multi-tenant-login-design.md](./multi-tenant-login-design.md)

**表结构**：
- `tenants` - 租户表
- `users` - 用户表
- `roles` - 角色表
- `user_roles` - 用户角色关联表

**核心设计**：
- 用户属于单个租户，通过 `tenant_id` 绑定
- Casbin `(username, tenantCode, roleCode)` 三元组权限控制
- Token 携带 `roles` 数组，包含 `super_admin` 即为超管

### 1.2 菜单系统

> 详细设计见 [menu-system-design.md](./menu-system-design.md)

**表结构**：
- `menus` - 菜单元数据（名称、路径、组件等）
- `tenant_menus` - 租户菜单边界
- `roles` - 角色定义
- `permissions` - 权限点定义（MENU/BUTTON/API）

### 1.3 字典表

> 详细设计见 [dict-system-design.md](./dict-system-design.md)

**表结构**：
- `dict_types` - 字典类型（系统字典所有租户共享）
- `dict_items` - 字典项

### 1.4 组织表 (organizations)

```sql
CREATE TABLE organizations (
    org_id VARCHAR(20) PRIMARY KEY,
    tenant_id VARCHAR(20) NOT NULL,
    parent_id VARCHAR(20),
    org_name VARCHAR(100) NOT NULL,
    org_code VARCHAR(50),
    leader_id VARCHAR(20),
    phone VARCHAR(20),
    description TEXT,
    sort INT DEFAULT 0,
    status SMALLINT DEFAULT 1,
    created_at BIGINT NOT NULL DEFAULT 0,
    updated_at BIGINT NOT NULL DEFAULT 0,
    deleted_at BIGINT DEFAULT 0,
    INDEX idx_tenant (tenant_id),
    INDEX idx_parent (parent_id)
);
```

---

## 2. scopes.go 改造

```go
const (
    TenantModeAuto   = "auto"   // 自动添加 tenant_id 过滤（默认）
    TenantModeManual = "manual" // 手动控制 WHERE 条件
)

// ManualTenantMode 设置手动模式，Repository 完全控制 WHERE
func ManualTenantMode(ctx context.Context) context.Context {
    return context.WithValue(ctx, tenantModeKey, TenantModeManual)
}
```

**查询优先级**：
1. `SkipTenantCheck` → 不添加任何过滤
2. `TenantModeManual` → Repository 手动控制 WHERE
3. 默认 (`TenantModeAuto`) → 自动添加 `WHERE tenant_id = ?`

---

## 3. API 设计

### 超管操作

```
POST   /api/v1/system/menus/assign             分配菜单给租户
DELETE /api/v1/system/menus/unassign           撤销分配
```

### 租户操作

```
GET    /api/v1/menus                           获取菜单（系统+自定义）
POST   /api/v1/menus                           创建自定义菜单

GET    /api/v1/dict-types                      获取字典类型
POST   /api/v1/dict-types                      创建自定义字典

GET    /api/v1/orgs                            获取组织结构
POST   /api/v1/orgs                            创建组织
```

### 登录

```
POST   /api/v1/:tenant_code/login              登录（路径携带租户code）
```

---

## 4. 常量定义

```go
package constants

const (
    // 租户
    DefaultTenantID   = "000000000000000000" // 默认租户ID（18个零）
    DefaultTenantCode = "default"            // 默认租户code
    SuperAdminRoleCode = "super_admin"       // 超管角色code

    // 用户类型
    UserTypeUser        = 1 // 普通用户
    UserTypeTenantAdmin = 2 // 租户管理员
    UserTypeSuperAdmin  = 3 // 超级管理员
)
```

---

## 5. 实现清单

### 用户角色系统（见 [multi-tenant-login-design.md](./multi-tenant-login-design.md)）
- [ ] `internal/model/tenant.go`
- [ ] `internal/model/user.go`
- [ ] `internal/model/role.go`
- [ ] `internal/repository/user_repo.go`
- [ ] `internal/repository/role_repo.go`
- [ ] `internal/service/auth_service.go`
- [ ] `internal/middleware/tenant.go`
- [ ] `internal/middleware/auth.go`

### 菜单系统（见 [menu-system-design.md](./menu-system-design.md)）
- [ ] `internal/repository/menu_repo.go`
- [ ] `internal/repository/tenant_menu_repo.go`
- [ ] `internal/repository/permission_repo.go`
- [ ] `internal/service/menu_service.go`
- [ ] `internal/service/role_service.go`

### 字典系统（见 [dict-system-design.md](./dict-system-design.md)）
- [ ] `internal/repository/dict_type_repo.go`
- [ ] `internal/repository/dict_item_repo.go`
- [ ] `internal/service/dict_service.go`

---

## 6. 待补充功能

🟡 中优先级
| 模块 | 说明 | 复杂度 |
|------|------|--------|
| 通知中心 | 站内消息、邮件/短信通知、通知模板管理 | 中 |
| 文件管理 | 文件上传下载、租户隔离存储、文件分享 | 低 |
| API管理 | API密钥管理、API限流/鉴权、API文档 | 中 |

🟢 低优先级
| 模块 | 说明 | 复杂度 |
|------|------|--------|
| 定时任务 | 任务调度管理、执行监控、任务日志 | 低 |
| 监控告警 | 系统性能监控、异常告警、资源使用监控 | 高 |
| 备份恢复 | 数据备份、数据恢复、租户数据迁移 | 高 |
| 租户管理 | 租户注册/开通、套餐/版本管理 | 低 |
