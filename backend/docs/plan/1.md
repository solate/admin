...
CREATE TABLE menus (
    ...
    source_type VARCHAR(10) NOT NULL,       -- 'SYSTEM' | 'CUSTOM'
...
CREATE TABLE tenant_menu (
    ...
    tenant_id VARCHAR(20) NOT NULL,
    template_id VARCHAR(20) NOT NULL,       -- 系统菜单ID
    is_enabled BOOLEAN DEFAULT TRUE,
    assigned_at BIGINT NOT NULL,
    assigned_by VARCHAR(100),
    created_at BIGINT NOT NULL DEFAULT 0,
    deleted_at BIGINT DEFAULT 0,
    UNIQUE KEY uk_tenant_menu (tenant_id, template_id, deleted_at)
);
```

**说明**：
- 移除 `override_name` 和 `override_icon`（覆盖功能通过租户创建自定义菜单记录实现）
- 统一使用 `BIGINT` 时间戳
- 添加软删除字段 `deleted_at`
...
CREATE TABLE dict_items (
    ...
    updated_at BIGINT NOT NULL DEFAULT 0,
    deleted_at BIGINT DEFAULT 0,
    INDEX idx_type_tenant (type_id, tenant_id),
    INDEX idx_tenant (tenant_id)
);
```

**注意**：字典系统不需要分配表，所有租户都可以使用系统字典，租户可以选择覆盖显示文本。详细设计见 [dict-system-design.md](./dict-system-design.md)。

### 1.4 组织表 (organizations) - 独立设计
...
- [x] `pkg/constants/system.go` 添加 `DefaultTenantID = "000000000000000000"`
- [ ] 菜单表：`tenant_id`, `parent_id`, `name`, `path`, `sort`
- [ ] 新建 `tenant_menus` 表（租户菜单边界）
- [ ] 角色表：`tenant_id`, `name`, `code`, `type`, `parent_role_code`
- [ ] 字典类型表：`tenant_id`, `type_code`, `type_name`
- [ ] 字典项表：`type_id`, `tenant_id`, `label`, `value`, `sort`
- [ ] 组织表：`tenant_id`, `parent_id`, `org_name`, `org_code`

### 后端代码

#### 菜单系统（见 [menu-system-design.md](./menu-system-design.md)）
- [ ] `internal/repository/menu_repo.go` 菜单查询
- [ ] `internal/repository/tenant_menu_repo.go` 租户菜单边界
- [ ] `internal/service/menu_service.go` 创建、分配、覆盖逻辑
- [ ] `internal/router/` 添加超管分配接口

#### 字典系统（见 [dict-system-design.md](./dict-system-design.md)）
- [ ] `internal/repository/dict_type_repo.go` 字典类型
- [ ] `internal/repository/dict_item_repo.go` 字典项
- [ ] `internal/service/dict_service.go` 创建、覆盖、恢复逻辑
- [ ] `internal/router/` 添加超管字典接口

#### 角色权限系统（见 [menu-system-design.md](./menu-system-design.md)）
- [ ] `internal/repository/role_repo.go` 角色管理
- [ ] `internal/service/role_service.go` 继承、自定义角色
- [ ] `internal/service/auth_service.go` 权限检查



🟡 中优先级（常用功能）
模块	缺少的设计内容	复杂度
通知中心	站内消息、邮件/短信通知、通知模板管理	中
文件管理	文件上传下载、租户隔离存储、文件分享	低
API管理	API密钥管理、API限流/鉴权、API文档（Swagger）	中
🟢 低优先级（可根据需要补充）
模块	缺少的设计内容	复杂度
定时任务	任务调度管理、执行监控、任务日志	低
监控告警	系统性能监控、异常告警、资源使用监控、租户用量统计	高
备份恢复	数据备份、数据恢复、租户数据迁移	高
租户管理	租户注册/开通、套餐/版本管理	低
系统配置	系统参数配置、租户级配置、配置版本管理	低
前端个性化	主题配置、布局配置、首页配置	低
