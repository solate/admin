...
CREATE TABLE tenants (
    ...
    code VARCHAR(50) NOT NULL, -- 租户编码（全局唯一，可用于二级域名或URL路径，如：tenant_shanghai）
...
COMMENT ON COLUMN tenants.code IS '租户编码(全局唯一业务标识)';
...
CREATE TABLE roles (
    ...
    code VARCHAR(50) NOT NULL, -- 角色编码 (如: hr_manager)
...
CREATE TABLE permissions (
    ...
    code VARCHAR(100) NOT NULL, -- 权限标识符 (如: sys:user:view)
...
CREATE UNIQUE INDEX idx_permissions_tenant_code ON permissions(tenant_id, code) WHERE deleted_at = 0;
