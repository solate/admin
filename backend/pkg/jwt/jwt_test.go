...
func TestGenerateTokenPairAndParse(t *testing.T) {
	...
	pair, err := GenerateTokenPair("tenant-1", "tenant-1", "user-1", "user-1", []string{"role-1"}, []string{"role-id-1"}, cfg)
	if err != nil {
...
func TestParseTokenExpired(t *testing.T) {
	...
	token, err := generateToken("tenant-1", "tenant-1", "user-1", "user-1", []string{"role-1"}, []string{"role-id-1"}, "token-1", expiredCfg.AccessExpire, secret, expiredCfg.Issuer)
	if err != nil {
